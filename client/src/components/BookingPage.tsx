...
import { formatTime, timezone, utcDateString } from '../lib/format';
...
export default function BookingPage({ eventType, owner, onBack }: BookingPageProps) {
  ...
  // Guards against out-of-order responses when the user clicks dates quickly.
  const requestSeq = useRef(0);

  // Logic generated by AI Agent: full booking window on mount, then a
  // date-scoped refetch (?date=) every time another day is selected, so the
  // grid always reflects fresh availability from the server (bug B1).
  useEffect(() => {
    const requestId = ++requestSeq.current;
    let cancelled = false;
    setLoading(true);
    listSlots(eventType.id)
      .then((data) => {
  ...
  // Fetches only the selected day and merges it back into the window so the
  // calendar keeps showing every day while the right panel refreshes.
  const selectDate = useCallback(
    (date: string) => {
      setSelectedDate(date);
      const requestId = ++requestSeq.current;
      setLoading(true);
      listSlots(eventType.id, date)
        .then((data) => {
          if (requestId !== requestSeq.current) return;
          setSlots((prev) => [...prev.filter((s) => s.date !== date), ...data.slots]);
          setError(null);
        })
        .catch((err) => {
          if (requestId !== requestSeq.current) return;
  ...
  // Group slots by the server's UTC calendar date (`slot.date`) — the same key
  // the API filters on with ?date= — so client and server always agree.
  ...
  const today = useMemo(() => utcDateString(new Date()), []);
  ...
  const handleBooked = useCallback(() => {
    setBookingSlot(null);
    // Refresh the selected day so the booked slot turns unavailable.
    if (selected?.date) selectDate(selected.date);
  }, [selectDate, selected]);
          ...
          {grid.map((cell) =>
            cell.empty ? (
              ...
              <button
                key={cell.key}
                type="button"
                onClick={() => selectDate(cell.day.date)}
